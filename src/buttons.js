...
let barcounter = 0; //counts how many timer bars exist
let gamewinner = 0; //0 if no one has won, 1 if has
let newbuttonpress = 1;
let gamertag = "x";
let buttonid = "If this shows, something is broken"; //defined in button press for function buttonActivities
let squareid = "If this shows, something is broken";
let counter = {
...
function buttonActivities() {
  ...
  if (
    ...
    document.getElementById(squareid).className = printed;
  }
  wincheck();
  gamerchange();
  printed = gamertag;
...
function timebar() {
  let elem = document.getElementById("underpar");
  let barwidth = 0;
  let time = 10 - barwidth / 10;
  ...
  if (elem != null) {
    function timer() {
      if (gamewinner === 1) {
        ...
        elem.innerHTML = gamertag + ` ` + time + `s`;
        gamerchange();
        id = null;
        barwidth = null;
        timebar();
      } else if (newbuttonpress === 0) {
        elem.style.width = barwidth + "%";
        elem.innerHTML = gamertag + ` ` + time + `s`;
        newbuttonpress = 1;
        clearInterval(id);
        ...
        elem.className = gamertag;

        if (barcounter >= 2) {
          //added so bars won't stack
          clearInterval(id);
          barcounter = 1;
        }
      }
